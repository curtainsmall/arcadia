    ...
    _gl_texture2d(viewport_size)
{
    ...
    bind();
    ARCADIA_GL_CALL(glFramebufferTexture(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, _gl_texture2d.get_id(), 0));
    unbind();
...
{
    ARCADIA_GL_CALL(glDeleteFramebuffers(1, &_gl_framebuffer_id));
}

arcadia::gl_framebuffer::gl_framebuffer(self_type&& rhs) noexcept:
    _gl_texture2d(std::move(rhs._gl_texture2d))
{
    _gl_framebuffer_id = rhs._gl_framebuffer_id;
    rhs._gl_framebuffer_id = 0;
}
...
{
    if(_gl_framebuffer_id == 0)
    {
        throw arcadia::gl_invalid{ "Cannot bind null OpenGL frambuffer" };
    }

    ARCADIA_GL_CALL(glBindFramebuffer(GL_FRAMEBUFFER, _gl_framebuffer_id));
}
